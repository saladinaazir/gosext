...
function Pantheon:__init()
    self.Q1 = {Hitchance = _G.HITCHANCE_HIGH, Type = _G.SPELLTYPE_LINE, Delay = 0.25, Radius = 60, Range = 600, Speed = math.huge, Collision = false}
    self.Q2 = {Hitchance = _G.HITCHANCE_HIGH, Type = _G.SPELLTYPE_LINE, Delay = 0.25, Radius = 60, Range = 1200, Speed = 2700, Collision = true, MaxCollision = 0, CollisionTypes = {_G.COLLISION_YASUOWALL}}
    self.W = {Range = 600}
    self.E = {Range = 250}
    self.R = {Range = 5500}
...
function Pantheon:LoadMenu()
	...
	self.tyMenu.Combo:MenuElement({name = "Use w on:", id = "useon", type = _G.MENU})
    ...
    self.tyMenu.Combo:MenuElement({id = "UseW", name = "[W]", value = false, toggle=false, key=string.byte("S")})
...
function Pantheon:Tick()
	...
	if self.tyMenu.Combo.UseW:Value() then
		...
		if target and self.tyMenu.Combo.UseW:Value() and  self.tyMenu.Combo.useon[target.charName] and self.tyMenu.Combo.useon[target.charName]:Value() then
