...
function Kaisa:LoadMenu()
    ...
    self.tyMenu.Combo:MenuElement({id = "UseW", name = "[W]", value = false, toggle=true, key=string.byte("S")})
...
function Kaisa:Draw()
	...
	if self.tyMenu.Combo.UseW:Value() then
        Draw.Text("W: On ",30,myHero.pos:To2D(),Draw.Color(255 ,0,255,0))
    else
        Draw.Text("W: Off ",30,myHero.pos:To2D(),Draw.Color(255 ,255,0,0))
