...
function Tristana:LoadMenu()
    ...
    self.tyMenu:MenuElement({type = MENU, id = "Combo", name = "Combo"})
		...
		self.tyMenu.Combo:MenuElement({id = "rAA", name = "min aas needed to kill before using R", value = 2, min = 0, max =5, step = 0.5})
...
function Tristana:IsInAutoAttackRange(unit)
...
function Tristana:Combo()
    ...
    if target and self.tyMenu.Combo.E:Value() and target.health > (DamageLib:CalculateDamage(myHero, target, _G.SDK.DAMAGE_TYPE_PHYSICAL ,myHero.totalDamage) * self.tyMenu.Combo.eAA:Value()) then
        if Ready(_E) and lastE + 20 < GetTickCount()  then
			print(myHero.totalDamage)
