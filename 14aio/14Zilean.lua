...
local function cantkillr(unit)
	...
	for i = 0, unit.buffCount do
		...
		if buff.name:lower():find("undyingrage") and buff.count==1 and buff.duration>0.2 then
			return true
		end
		if buff.name:lower():find("kindredrnodeathbuff") and buff.count==1  then
			return true
		end	
		if buff.name:lower():find("chronoshift") and buff.count==1 then
			return true
		end			
		if  buff.name:lower():find("willrevive") and buff.count==1 then
			return true
		end
...
function Zilean:UltCalcs(unit,ally)
    ...
    if unit.activeSpell.target == ally.handle and unit.activeSpell.isChanneling == false and unit.totalDamage and unit.critChance then
        ...
        CheckDmg = (unit.totalDamage + (unit.totalDamage*unit.critChance))*0.7
    else
        --PrintChat("Spell")
    ...
    for i = 1, #Allys do
        ...
        if self.tyMenu.Auto.Ron[ally.charName] and self.tyMenu.Auto.Ron[ally.charName]:Value() then
            if IsValid(ally) and GetDistanceSquared(ally.pos, myHero.pos) < self.R.Range ^ 2 then
			    for i, enemy in pairs(EnemyHeroes) do
				if GetDistanceSquared(enemy.pos, ally.pos) < 800 ^ 2 then
					local IncDamage = self:UltCalcs(enemy,ally)
			--		print(IncDamage)
					if cantkillr(ally)==false and ( (ally.health / ally.maxHealth * 100 < self.tyMenu.Auto.Rhp:Value() and self:GetEnemyAround(ally) > 0 ) or (IncDamage> ally.health and ally.health / ally.maxHealth<0.5) ) then
						if ally==myHero then
							Control.KeyDown(18)
