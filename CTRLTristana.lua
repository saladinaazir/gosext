...
function Tristana:LoadMenu()
    self.tyMenu = MenuElement({type = MENU, id = "14Tristana", name = "CTRL Tristana"})
	self.tyMenu:MenuElement({name = " ", drop = {"Reworked 14tristana"}})
    self.tyMenu:MenuElement({type = MENU, id = "Combo", name = "Combo"})
		...
		self.tyMenu.Combo:MenuElement({id = "RKey", name = "[R] Manual Key", key = string.byte("S"), toggle = false, value = false})
        self.tyMenu.Combo:MenuElement({id = "E", name = "E", value = true, toggle= true })
        self.tyMenu.Combo:MenuElement({id = "AA", name = "Force Attack Target have E Buff ", value = true})
		self.tyMenu.Combo:MenuElement({id = "eAA", name = "min aas needed to kill before using E", value = 4, min = 1, max =8, step = 1})
		self.tyMenu.Combo:MenuElement({id = "rAA", name = "min aas needed to kill before using R (KS)", value = 1.5, min = 0, max =5, step = 0.25})
    ...
    self.tyMenu:MenuElement({type = MENU, id = "Anti", name = "Anti Gap"})
        self.tyMenu.Anti:MenuElement({id = "AntiR", name = "[R] Anti Gap R", value = false})
    ...
    if args.Process then
        if orbwalker.Modes[0] then
            if self.tyMenu.Combo.useon[args.Target.charName] and self.tyMenu.Combo.useon[args.Target.charName]:Value() then
                if Ready(_Q) and lastQ + 300 < GetTickCount() and self.tyMenu.Combo.Q:Value() and not Ready(_E) then
            ...
            if self.tyMenu.Combo.AA:Value() then
                ...
                if self.ETarget and IsValid(self.ETarget) and self.ETarget.type == myHero.type and Tristana:IsInAutoAttackRange(self.ETarget) then
...
function Tristana:Tick()
	...
	if self.tyMenu.Combo.RKey:Value() then
		local target = _G.SDK.TargetSelector:GetTarget(1000);
		if target and Tristana:IsInAutoAttackRange(target) and Ready(_R) then
...
function Tristana:IsInAutoAttackRange(target)
	 local AArange = myHero.range + myHero.boundingRadius
	if	myHero.pos:DistanceTo(target.pos) < AArange then
		return true
	end
	return false
...
function Tristana:Combo()
    ...
    if target and Ready(_E) and self.tyMenu.Combo.E:Value() and( orbwalker:CanMove() or HasBuff(myHero,"TristanaW")) and target.health > (DamageLib:CalculateDamage(myHero, target, _G.SDK.DAMAGE_TYPE_PHYSICAL ,myHero.totalDamage) * self.tyMenu.Combo.eAA:Value()) then
        if  lastE + 20 < GetTickCount()  then
...
function Tristana:KS()
    ...
    if Ready(_R) and lastR + 180 < GetTickCount() and self.tyMenu.Auto.UseR:Value()  then
        for i = 1, #Enemys do
            ...
            if target.health < self:GetRDmg(target) and IsValid(target) and target.health > (DamageLib:CalculateDamage(myHero, target, _G.SDK.DAMAGE_TYPE_PHYSICAL ,myHero.totalDamage) * self.tyMenu.Combo.rAA:Value()) and
