...
-- stylua: ignore start
	...
	Menu.q_SemiQKey = Menu.q:MenuElement({id = 'SemiQKey', name = 'Semi Q Key', value = true, key=string.byte("S"), toggle = false})
...
-- stylua: ignore start
	...
	function Champion:QAuto()
		if not Menu.q_auto_enabled:Value() and not Menu.q_SemiQKey:Value() then
			return
		end
		local enemies = {}
		for i, unit in ipairs(self.QTargets) do
...
-- stylua: ignore start
    ...
    Menu.w_combo = Menu.w:MenuElement({id = 'combo', name = 'Combo', value = true,toggle=true, key=string.byte("s")})
    ...
    Menu.d_autoq_enabled = Menu.d.autoq:MenuElement({id = "enabled", name = "Enabled", value = false})
...
local function cantkill(unit,kill,ss,aa)
	...
	function Champion:OnDraw()
		if Menu.d_autoq_enabled:Value() then
			...
			if Menu.w_combo:Value() then
				Draw.Text("W on", Menu.d_autoq_size:Value(), posX, posY, Draw.Color(255, 000, 255, 000))
			else
				Draw.Text("W off", Menu.d_autoq_size:Value(), posX, posY, Draw.Color(255, 255, 000, 000))
			end
		end
	end
end
...
-- stylua: ignore start
    ...
    Menu.e_xstacks = Menu.e:MenuElement({id = 'xstacks', name = 'harassX Stacks', value = 6, min = 1, max = 6, step = 1})
    Menu.e_xenemies = Menu.e:MenuElement({id = 'xenemies', name = 'harassX Enemies', value = 1, min = 1, max = 5, step = 1})
	
	Menu.e_cxstacks = Menu.e:MenuElement({id = 'cxstacks', name = 'comboX Stacks', value = 6, min = 1, max = 6, step = 1})
    Menu.e_cxenemies = Menu.e:MenuElement({id = 'cxenemies', name = 'comboX Enemies', value = 1, min = 1, max = 5, step = 1})
	...
	Menu.e_ks_enabledfs = Menu.e.ks:MenuElement({id = 'enabledfs', name = 'onlywhenfsup', value = true})
...
local function hasfs()
	for i = 0, myHero.buffCount do
		...
		if buff and buff.name:lower():find("firststrikeavailable") and  buff.count==1 then 
	...
	function Champion:OnTick()
		...
		-- if (fsproc+3000)>GetTickCount() then
			-- print(fsproc)
		-- else
			-- print("no")
		-- end
		...
		-- if (self.IsAttacking or self.CanAttackTarget or self.AttackTarget) then
			...
			-- return
		-- end
		-- if target==nil or not GG_Data:IsInAutoAttackRange(myHero, target) then
			-- return
		-- end
		-- self:WLogic()
	end
	...
	function Champion:OnDraw()
		...
		for _, hero in ipairs(self.ETargets) do
			if EBuffs[hero.networkID] then
				...
				local eduration = EBuffs[hero.networkID].duration
				if ecount > 0 then
					...
					local passivedamage = (1+myHero.ap * 0.03)*ecount*eduration
					local totalDamage =passivedamage
					if GG_Spell:IsReady(_E)then
					 totalDamage=totalDamage+(physDamage+magicDamage)
					end
					if totalDamage >= (hero.health + (eduration * hero.hpRegen)) then
						Draw.Circle(hero.pos, 200, 20, INV_CIRCLE_COLOR)
					end
	...
	function Champion:WLogic()
		...
		-- if target==nil then
			-- return
		-- end
		-- if not GG_Data:IsInAutoAttackRange(myHero, target)  then
			-- return
		-- end
		...
		for _, hero in ipairs(self.ETargets) do
			if EBuffs[hero.networkID] then
				...
				if ecount > 0 then
					...
					local magicDamage = GG_Damage:CalculateDamage(myHero, hero, DAMAGE_TYPE_MAGICAL, rawMagic)
					local passivedamage = (1+myHero.ap * 0.03)*ecount*eduration
					local totalDamage = physDamage + magicDamage+passivedamage
					if (fsproc+2850)>GetTickCount() then
						totalDamage=totalDamage*1.09
					end

					if totalDamage >= hero.health + (eduration*1 * hero.hpRegen) and ecount>3 and (totalDamage/hero.health)<3 and Menu.e_useon[hero.charName] and ((fsproc+2710)>GetTickCount() or not Menu.e_ks_enabledfs:Value()) then
	...
	function Champion:ECombo()
		...
		for _, hero in ipairs(self.ETargets) do
			if EBuffs[hero.networkID] then
				local ecount = EBuffs[hero.networkID].count
				if ecount > 0 and ecount >= Menu.e_cxstacks:Value() then
					cxenemies = cxenemies + 1
				end
			end
		end		
		if (cxenemies >= Menu.e_cxenemies:Value() and self.IsCombo) or (xenemies >= Menu.e_xenemies:Value() and self.IsHarass and (fsproc+2720)>GetTickCount() and (fsproc+2560)<GetTickCount())  then
			Utils:Cast(HK_E)
		end
	end
	-- r combo
...
-- stylua: ignore start
	...
	Menu.w_combo = Menu.w:MenuElement({id = "combo", name = "prio target with 2 w stacks", value = true})
	...
	local function OnPreAttack1(args)
		-- if not process then
			-- args.Process = false
		-- end
		 if GG_Orbwalker.Modes[ORBWALKER_MODE_COMBO] and Menu.w_combo:Value()  then          
			local heros = GG_Object:GetEnemyHeroes(_G.SDK.Data:GetAutoAttackRange(myHero), true, true, true, true)
				for i = 1, #heros do
                    local target = heros[i]
					local buff = Hasbuff(target, "VayneSilveredDebuff")
            
                    if buff and buff.count == 2 and ValidTarget(target) and GG_Data:IsInAutoAttackRange(myHero, target)  then
                        args.Target = target
			...
			if enemy.distance < EPrediction.Range + self.BoundingRadius + enemy.boundingRadius - 35 then
				...
				if useon and useon:Value() then
					...
					if
						...
						and (enemy.charName~= "Fiora" or enemy:GetSpellData(_W).currentCd>0.3) 
					then
