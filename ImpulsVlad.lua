...
if myHero.charName == "Vladimir" then
    ...
    function Vladimir:Menu()
		...
		Menu.Q:MenuElement({id = "useaa", name = "use aa in combo", value = true, toggle=true, key=string.byte("K")})
		...
		Menu.Burst:MenuElement({id = "Flashhelperenabled", name = "will use 'O' to flash with ggorb flashhelper", value = false})
        Menu.Burst:MenuElement({id = "Key", name = "Burst Key", key = string.byte("T")})
		Menu.Burst:MenuElement({id = "Key2", name = "smart e channel key (makes proper combo ez)", key = string.byte("S")})
		Menu.Burst:MenuElement({id = "Key3", name = "smart e channel key (this one early recasts)", key = string.byte("A")})
		Menu.Burst:MenuElement({id = "Key2noslow", name = "autocancel e if fully charged", value=false,toggle=true})
        --
 ...
 keythreetime=0
   keythree=false  
    ...
    function Vladimir:OnTick()
		...
		if Menu.Burst.Key3:Value() then
			if Ready(_E) then
				keythree=true
				keythreetime=Game.Timer()
				KeyDown(HK_E)
			end
        end
	-- ebuf=false or GetBuffData(myHero, "VladimirE")
		-- if Menu.Burst.Key2:Value() and  (ebuf and ebuf.duration<0.6) and keytwotime+0.3 < Game.Timer() then	
			-- print("cancel")
			-- keytwo=false
			-- KeyUp(HK_E)
        -- end
    ...
    function Vladimir:OnDash(unit, unitPos, unitPosTo, dashSpeed, dashGravity, dashDistance)
        ...
        if IsValidTarget(unit) and GetDistance(unitPosTo) < 400 and unit.team == TEAM_ENEMY and IsFacing(unit, myHero) then --Gapcloser
	...
	if  #GetEnemyHeroes(1000) > 1 or self.E:IsReady() or self.Q:IsReady() or not Menu.Q.useaa:Value()  then
		_G.SDK.Orbwalker:SetAttack(false)
	end
    ...
    function Vladimir:Clear()
        ...
        if self.Q:IsReady() and (jCheckQ or lCheckQ) then
            ...
            for i = 1, #minions do
                ...
                if (minion.health <= self.Q:GetDamage(minion) or (HasBuff(myHero, "vladimirqfrenzy") and self.Q:GetDamage(minion,2) >= minion.health))  or minion.team == TEAM_JUNGLE then
                    self.Q:Cast(minion)
                    return
                end
            end
    ...
    function Vladimir:LastHit()
        if self.Q:IsReady() and Menu.Q.LastHit:Value() then
            ...
            for i = 1, #minions do
                local minion = minions[i]
                if minion.health <= self.Q:GetDamage(minion) or (HasBuff(myHero, "vladimirqfrenzy") and self.Q:GetDamage(minion,2) >= minion.health) then --check if Q dmg is right
                    self.Q:Cast(minion)
                    return
                end
            end
    ...
    function Vladimir:KillSteal()
        if (Menu.Q.KS:Value() and self.Q:IsReady()) then
            for i = 1, #self.enemies do
                ...
                if enemy and self.Q:GetDamage(enemy) >= enemy.health or (enemy and HasBuff(myHero, "vladimirqfrenzy") and self.Q:GetDamage(enemy,2) >= enemy.health)  then
                    self.Q:Cast(self.target); return
                end
            end
        end
...
function GetHeroTarget(range)
    ...
    function Vladimir:LogicE()
        ...
        --
		local ePrediction = GGPrediction:SpellPrediction({Type = GGPrediction.SPELLTYPE_LINE, Delay = 0, Radius = 10, Range = self.E.Range, Speed = 4000, Collision = true, CollisionTypes = {GGPrediction.COLLISION_MINION,GGPrediction.COLLISION_YASUOWALL}})
        ...
        for i = 1, #enemies do
			...
			ePrediction:GetPrediction(target, myHero)
                ...
                if GetDistance(tP2, pP2) > eRange and target==GetHeroTarget(600) then
                    leaving = leaving + 1
                end
		...
		ebuf=false or GetBuffData(myHero, "VladimirE")
		if ebuf then	print(ebuf.duration) end
        if entering <= leaving and (willHit > 0 or entering == 0) and IsKeyDown(HK_E) then
            if ((leaving > 0) or (#GetEnemyHeroes(eRange+25)==0 and self.mode == 1 and keytwo==false and keythree==false)) and (Ready(_Q) or (ebuf and ebuf.duration<0.6)) then 
                KeyUp(HK_E) --release skill
				keytwo=false
				keythree=false
				--print("1")
            end
			if ebuf and ebuf.duration<0.6 and willHit > 0 and (self.mode == 1 or keytwo) then
			keytwo=false
			keythree=false
			KeyUp(HK_E)
				--print("2")--release skill
			end
        end
		if ebuf and ebuf.duration<0.6 and IsKeyDown(HK_E) and (keythree or (keytwo and Menu.Burst.Key2noslow:Value())) then
			keytwo=false
			keythree=false
			KeyUp(HK_E)
		end
    ...
    function Vladimir:LogicW()
        if self.W:IsReady() and not self.Q:IsReady() and not self.E:IsReady() and not self.R:IsReady() and ((self.mode == 1 and Menu.W.Combo:Value()) or (self.mode == 2 and Menu.W.Harass:Value())) then
            local nearby = GetEnemyHeroes(600)
            --
            for i = 1, #nearby do
                local enemy = nearby[i]
