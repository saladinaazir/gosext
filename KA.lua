...
--Main Menu
Annie.Menu = MenuElement({type = MENU, id = "KA", name = "KA", leftIcon = AnnieIcon})
...
function Annie:LoadMenu()                     	
	...
	self.Menu.Combo.RSettings:MenuElement({id = "RBuffer", name = "R AOE buffer", value = 30, min = 0, max = 75, step = 1,tooltip = "larger value will ult people closer together"})
	self.Menu.Combo.RSettings:MenuElement({id = "DontSoloUlt", name = "Don't Use Solo R on...", type = MENU})
	_G.SDK.ObjectManager:OnEnemyHeroLoad(function(args)
		self.Menu.Combo.RSettings.DontSoloUlt:MenuElement({id = args.charName, name = args.charName, value = true})
	end)
	...
	self.Menu.Combo.NinjaCombo:MenuElement({id = "Key2", name = "noflash key", key = string.byte("A")})
	self.Menu.Combo.NinjaCombo:MenuElement({id = "RBuffer", name = "R AOE buffer", value = 30, min = 0, max = 200, step = 1,tooltip = "larger value will ult closer to center of target"})
	-- Harass
...
function Annie:DebugCluster()
	local RBuffer = self.Menu.Combo.NinjaCombo.RBuffer:Value()
...
function Annie:NinjaCombo()
	...
	--Flash Check
	if(self.Menu.Combo.NinjaCombo.UseFlash:Value() and self.Menu.Combo.NinjaCombo.Key:Value()==true) then
		if myHero:GetSpellData(SUMMONER_1).name == "SummonerFlash" and Ready(SUMMONER_1) then
			canFlash = true
			flashSlot = HK_SUMMONER_1
		elseif myHero:GetSpellData(SUMMONER_2).name == "SummonerFlash" and Ready(SUMMONER_2) then
...
function Annie:Draw()
	...
	if((self.Menu.Combo.NinjaCombo.Key:Value() or self.Menu.Combo.NinjaCombo.Key2:Value()) and self.Menu.Drawings.DrawNinjaComboStatus:Value()) then
