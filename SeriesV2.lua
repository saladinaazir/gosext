...
function Tryndamere:Logic()
    ...
    if Mode() == "Combo" or Mode() == "Harass" and target then
        ...
        elseif self:CanUse(_E, Mode()) and ValidTarget(target, ERange) and not CastingW and not (myHero.pathing and myHero.pathing.isDashing) and not _G.SDK.Attack:IsActive() then
            if self.Menu.ComboMode.UseESticky:Value() then
                if GetDistance(target.pos) > AARange+40 and (WasInRange or self.Menu.ComboMode.UseEGapClose:Value()) then 
                    self:UseE(target)
                end
...
function Tryndamere:UseE(unit)
    if self.Menu.ComboMode.UseEFast:Value() and IsReady(_E) then
        if  _G.SDK.Cursor.Step == 0 then
					 _G.SDK.Cursor:Add(string.byte("L"), unit.pos)
		end
    else
        local pred = _G.PremiumPrediction:GetPrediction(myHero, unit, ESpellData)
        if pred.CastPos and pred.HitChance > 0 and myHero.pos:DistanceTo(pred.CastPos) < 1150 then
            if not (myHero.pathing and myHero.pathing.isDashing) and IsReady(_E) then
				if  _G.SDK.Cursor.Step == 0 then
					 _G.SDK.Cursor:Add(string.byte("L"),  Vector(pred.CastPos):Extended(myHero.pos, -100))
				end
...
function Jax:Logic()
    ...
    if Mode() == "Combo" or Mode() == "Harass" and target then
        ...
        if not (myHero.pathing and myHero.pathing.isDashing) and not _G.SDK.Attack:IsActive() then
            if ValidTarget(target, ERange) and self:CanUse(_E, "Combo")  then
                Control.CastSpell(HK_E)
            elseif ValidTarget(target, EAARange) and self.Menu.ComboMode.UseEBlock:Value() then
...
function Corki:ManualRCast()
    ...
    if SmallRocket == false then
		R3Prediction = GGPrediction:SpellPrediction({Type = GGPrediction.SPELLTYPE_LINE, Delay = 0.175, Radius = 40, Range = 1500, Speed = 2000, Collision = false})
		R3Prediction:GetPrediction(target, myHero)
		if R3Prediction:CanHit(1) then
			Control.CastSpell(HK_R, R3Prediction.CastPosition)	
		end
    else
		R4Prediction = GGPrediction:SpellPrediction({Type = GGPrediction.SPELLTYPE_LINE, Delay = 0.175, Radius = 40, Range = 1300, Speed = 2000, Collision = false})
		R4Prediction:GetPrediction(target, myHero)
		if R4Prediction:CanHit(1) then
			Control.CastSpell(HK_R, R4Prediction.CastPosition)	
		end
    end
end
...
function Corki:Logic()
    ...
    if Mode() == "Combo" or Mode() == "Harass" and target then
        ...
        if self:CanUse(_R, Mode()) and ValidTarget(target, RRange)  and not _G.SDK.Attack:IsActive()  and not (myHero.pathing and myHero.pathing.isDashing)  then
            self:UseR(target)
        end
    else
        WasInRange = false
...
function Corki:UseQ(unit, hits)
   -- local pred = _G.PremiumPrediction:GetAOEPrediction(myHero, unit, QSpellData)
	QPrediction = GGPrediction:SpellPrediction({Type = GGPrediction.SPELLTYPE_CIRCLE, Delay = 0.25, Radius = 125, Range = 825, Speed = 1400, Collision = false})
...
function Corki:UseR(unit)
    local SmallRocket = _G.SDK.BuffManager:HasBuff(myHero, "corkimissilebarragenc")
    if SmallRocket == false then
		RPrediction = GGPrediction:SpellPrediction({Type = GGPrediction.SPELLTYPE_LINE, Delay = 0.175, Radius = 80, Range = 1300, Speed = 2000, Collision = true, CollisionTypes = {GGPrediction.COLLISION_MINION}})
		RPrediction:GetPrediction(unit, myHero)
		if RPrediction:CanHit(1+1) then
			Control.CastSpell(HK_R, RPrediction.CastPosition)	
		end
    else
		R1Prediction = GGPrediction:SpellPrediction({Type = GGPrediction.SPELLTYPE_LINE, Delay = 0.175, Radius = 80, Range = 1300, Speed = 2000, Collision = true, CollisionTypes = {GGPrediction.COLLISION_MINION}})
		R1Prediction:GetPrediction(unit, myHero)
		if R1Prediction:CanHit(1+1) then
