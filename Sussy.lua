...
function Lasthit()
	...
	for i = 1, #minions do
		...
		local ShouldAA = false
		if Champion:IsValid(minion) then
		 local value=mundoqdmg(minion)
			local hp = _G.SDK.HealthPrediction:GetPrediction(minion, 0.25+((GetDistanceSquared(myHero.pos, minion.pos))/(2000*2000)))
			if hp<value and hp>0 and (GetDistanceSquared(myHero.pos, minion.pos) > 210 * 210) then
					local prediction = _G.PremiumPrediction:GetPrediction(myHero, minion, {
					hitChance = 0.5,
					speed = 2000,
					range = 975,
			...
			Q    = { range = 650                                                                                                }         
			Q2   = { range = 800 , delay = 0.15, speed = 2000, width = 150, collision = false, aoe = true, type = "linear" }         
			...
			function geteDmg(minion)
				...
				local ebaseDmg  = (20 * elvl-10)*1.2
					
				local eDmg = (ebaseDmg+ myHero.totalDamage)*1.4
				return _G.SDK.Damage:CalculateDamage(myHero, minion, _G.SDK.DAMAGE_TYPE_PHYSICAL, eDmg) 
			end

			
			function mundoEcast(target)
				if target == nil then return end
				local pred = target:GetPrediction(Q2.speed, Q2.delay)
				if pred == nil then return end 
				local targetPos = LocalVector(myHero.pos):Extended(pred, Q2.range) 
				if Spells:IsReady(_E)  and ValidTarget(target, Q2.range) and GetDistance(myHero.pos, pred) <= Q2.range then 
					for i, minion in pairs(GetMinions()) do 
						if minion and not minion.dead and ValidTarget(minion,200) and geteDmg(minion) >= minion.health  then 
							local minionPos = LocalVector(myHero.pos):Extended(LocalVector(minion.pos), Q2.range)
							if GetDistance(targetPos, minionPos) <= 110 then 
								Control.CastSpell(HK_E)
            ...
            Callback.Add(
                ...
                function()
					...
					if Spells:IsReady(_E) and Menu.e_harass:Value() and (mode == "Harass" or mode == "Combo") and (lastETick+500)<GetTickCount() then
						local target = Orb:GetTarget(800)
						mundoEcast(target)
                    ...
                    if Spells:IsReady(_Q) then
					if Menu.q_farm:Value() and (mode == "LastHit" or mode=="Clear") then
						Lasthit()
					end
                        if Menu.q_killsteal:Value() then
                            local count = 0
            ...
            Orb:OnPostAttack(
                function()
                    local mode = Orb:GetMode()
                    if
                        Spells:IsReady(_E) and mode == "Combo" and Menu.e_combo:Value()  then
                        local target = Orb:GetTarget(300)
                        if target then
                            Control.CastSpell(HK_E)
                        end
