...
elseif myHero.charName == "Darius" then
    ...
    function Darius:Spells()
        ...
        self.R.GetDamage = function(spellInstance, enemy, stage)
            ...
            --
            return (125 * rLvl + 0.75 * myHero.bonusDamage)
        end
    end
    ...
    function Darius:Menu()
       ...
       _G.SDK.ObjectManager:OnEnemyHeroLoad(function(args)
		...
		--Menu.W:MenuElement({id = "Combo2", name = "Combo Mode", value = true})
        Menu.W:MenuElement({id = "Mana", name = "Min Mana %", value = 15, min = 0, max = 100, step = 1})
        Menu.W:MenuElement({name = " ", drop = {"Harass Settings"}})
        ...
        --E--
        Menu.E:MenuElement({name = " ", drop = {"Combo Settings"}})
        Menu.E:MenuElement({id = "Combo", name = "Combo Mode", value = false,  toggle=false, key=string.byte("a")})
        Menu.E:MenuElement({id = "Mana", name = "Min Mana %", value = 15, min = 0, max = 100, step = 1})
        ...
        --R--
        Menu.R:MenuElement({name = " ", drop = {"Combo Settings"}})
        Menu.R:MenuElement({id = "Combo", name = "Use on Combo", value = true, toggle=true, key=string.byte("S")})
        Menu.R:MenuElement({id = "Mana", name = "Min Mana %", value = 0, min = 0, max = 100, step = 1})
        Menu.R:MenuElement({name = " ", drop = {"Misc"}})
    ...
    function Darius:OnTick()
        ...
        self.target = GetTarget(self.Q.Range+200, 0)
        self.mode = GetMode()
        
		
		enemy1 =_G.SDK.TargetSelector:GetTarget(1000, _G.SDK.DAMAGE_TYPE_PHYSICAL)
		 -- if enemy1 and ( (not enemy1.charName== "Fiora") or enemy1:GetSpellData(_Q).currentCd>0.3) then
			 -- print("ok")
		 -- end

		if enemy1 and enemy1.charName== "Fiora" then
			if Ready(_E) and Menu.E.Combo:Value()  and enemy1:GetSpellData(_W).currentCd>0.1 and GotBuff(enemy1, "FioraW") ==0 then
				local EPrediction = GGPrediction:SpellPrediction({Type = GGPrediction.SPELLTYPE_CONE, Delay = 0.25, Radius = 50, Range = 535, Speed = MathHuge, Collision = false})
				  EPrediction:GetPrediction(enemy1, myHero)
				if EPrediction:CanHit(1+1) then
		...
		elseif enemy1 and Ready(_E) and Menu.E.Combo:Value()  then
			local EPrediction = GGPrediction:SpellPrediction({Type = GGPrediction.SPELLTYPE_CONE, Delay = 0.25, Radius = 50, Range = 535, Speed = MathHuge, Collision = false})
				  EPrediction:GetPrediction(enemy1, myHero)
				if EPrediction:CanHit(1+1) then
					Control.CastSpell(HK_E, EPrediction.CastPosition)
    ...
    function Darius:OnPreMovement(args) --args.Process|args.Target
        ...
        --Q Helper logic
        if self.moveTo then
			target=GetTarget(self.Q.Range+300, 0)
			qtime=0.71-(Game.Timer()-qstart)
			if qtime>0.4 then
				qtime=0.4
			end
			if qtime<0 then
			...
			if target and GetDistance(self.target:GetPrediction(huge,qtime))>360 and GetDistance(self.target:GetPrediction(huge,qtime))<410 then--GetDistance(self.moveTo) < 20 then
                  args.Process = true
    ...
    function Darius:OnDash(unit, unitPos, unitPosTo, dashSpeed, dashGravity, dashDistance)
        if ShouldWait() then return end
        if Menu.E.Auto:Value() and IsValidTarget(unit, self.E.Range) and GetDistance(unitPosTo) > 300 and unit.team == TEAM_ENEMY then -- and not IsFacing(unit, myHero) then
...
local function cantkill(unit,kill,ss,aa)
    ...
    function Darius:Auto()
        if self.enemies and (Menu.R.Auto:Value() or (Menu.R.Combo:Value() and self.mode == 1)) and self.R:IsReady() then
            for i = 1, #(self.enemies) do
				...
				print(_G.SDK.Damage:CalculateDamage(myHero, enemy, _G.SDK.DAMAGE_TYPE_TRUE, ( self.R:CalcDamage(enemy) * self:GetUltMultiplier(enemy)) ))
                if (_G.SDK.Damage:CalculateDamage(myHero, enemy, _G.SDK.DAMAGE_TYPE_TRUE, ( self.R:CalcDamage(enemy) * self:GetUltMultiplier(enemy)) ))>= enemy.health and not cantkill(enemy,true,true,false) then
                    self.R:Cast(enemy)
                    break
                end
            end
    ...
    function Darius:Combo()
        ...
        for i = 1, #(self.enemies) do
            ...
            if self.E:IsReady() and Menu.E.Combo:Value()  and distance <= self.E.Range then
			local EPrediction = GGPrediction:SpellPrediction({Type = GGPrediction.SPELLTYPE_CONE, Delay = 0.25, Radius = 50, Range = 535, Speed = MathHuge, Collision = false})
    ...
    function Darius:OnDraw()
        if Menu.Q.Auto:Value() and HasBuff(myHero, "dariusqcast") and self.target then
			if qstart+1.1<Game.Timer() then
			qstart=Game.Timer()
			print(Game.Timer())
			end
			
			qtime1=0.75-(Game.Timer()-qstart)
			if qtime1>0.4 then
				qtime1=0.4
			end
			if qtime1<0 then
            ...
            self.moveTo = self.target:GetPrediction(huge,qtime1):Extended(myHero.pos, ((self.Q.Radius + self.Q.Range) / 2))
        else
            self.moveTo = nil
        end
		
		if Menu.R.Combo:Value() then
...
local function getEnemyCount(range, unit)
	local count = 0
	for i, hero in ipairs(GetEnemyHeroes(range+100)) do
    ...
    function Olaf:Menu()
        --Q--
        Menu.Q:MenuElement({name = " ", drop = {"Combo Settings"}})
        Menu.Q:MenuElement({id = "Combo", name = "Use on Combo", value = false, toggle=true, key=string.byte("S")})
		Menu.Q:MenuElement({id = "overshoot", name = "q overshoot ", value = 50, min = 0, max = 150, step = 10})
        Menu.Q:MenuElement({id = "Mana", name = "Min Mana % ", value = 15, min = 0, max = 100, step = 1})
        Menu.Q:MenuElement({name = " ", drop = {"Harass Settings"}})
        Menu.Q:MenuElement({id = "Harass", name = "Use on Harass", value = true})
        Menu.Q:MenuElement({id = "ManaHarass", name = "Min Mana % ", value = 15, min = 0, max = 100, step = 1})
    ...
    function Olaf:Auto()
        if Menu.R.Auto:Value() and IsImmobile(myHero, Menu.R.Min:Value()) and getEnemyCount(500, myHero.pos)>=1 then
            self.R:Cast()
        end
    end
        ...
        if self.enemies then
            for i = 1, #self.enemies do
                ...
                elseif Menu.Q.KS:Value() and self.Q:IsReady() and self.Q:CalcDamage(enemy) >= enemy.health + enemy.shieldAD then
                    castSpellExtended({Type = _G.SPELLTYPE_LINE, Delay = 0.25, Radius = 55, Range = 1000, Speed = 1600, Collision = true, CollisionTypes = {_G.COLLISION_YASUOWALL}}, HK_Q,enemy,-150)
                end
            end
        end
    end
    ...
    function Olaf:Combo()
        ...
        if qTarget and Menu.Q.Combo:Value() and self.Q:IsReady() and ManaPercent(myHero) >= Menu.Q.Mana:Value()then
            castSpellExtended({Type = _G.SPELLTYPE_LINE, Delay = 0.25, Radius = 55, Range = 1000, Speed = 1800, Collision = true, CollisionTypes = {_G.COLLISION_YASUOWALL}}, HK_Q,qTarget, -Menu.Q.overshoot:Value())
        end
        --
    ...
    function Olaf:OnDraw()
        DrawSpells(self)
		if Menu.Q.Combo:Value() then
        Draw.Text("Q: On ",30,myHero.pos:To2D(),Draw.Color(255 ,0,255,0))
    else
        Draw.Text("Q: Off ",30,myHero.pos:To2D(),Draw.Color(255 ,255,0,0))
...
local HealthPrediction         = _G.SDK.HealthPrediction
    ...
    function Syndra:Spells()
        ...
        self.R.GetDamage = function(spellInstance, enemy, stage)
            ...
            --
            return (50 + 40 * rLvl + 0.17 * myHero.ap) * ammo
        end
    end
    ...
    function Syndra:Menu()
        ...
        Menu.R:MenuElement({id = "Combo", name = "Use on Combo", value = true})
        Menu.R:MenuElement({id = "KS", name = "Use on KS", value = true})
		Menu.R:MenuElement({id = "KS2", name = "R 100 splinters reached", value = false})
        ...
        --
		Menu.Q:MenuElement({id = "AAlevel", name = "Disable AA >= Level If Q/W/E ready", value = 6, min = 1, max = 19, step = 1})
        Menu.Q:MenuElement({id = "AAlevel2", name = "Disable AA", value = 6, min = 1, max = 19, step = 1})
		Menu.Q:MenuElement({id = "AAQ", name = "Disable AA if Q ready", value = true})
		...
		_G.SDK.ObjectManager:OnEnemyHeroLoad(function(args)
			...
			Menu.QE:MenuElement({id = charName, name = charName, value = true})
...
local function castSpell(spellData, hotkey, target)
	...
	function Syndra:castW(target)
          ...
          castSpell({Type = GGPrediction.SPELLTYPE_CIRCLE, Delay = 0.25, Radius = 200, Range = 925, Speed = 1450, Collision = false}, HK_W, target)
			self:UpdateBalls()
    ...
    function Syndra:Combo()
        ...
        if Menu.E.Combo:Value() and ManaPercent(myHero) >= Menu.E.Mana:Value() and self.E:IsReady()  then
            self:CastE(Menu.E.Pred:Value())
        end
        if Menu.W.Combo:Value() and ManaPercent(myHero) >= Menu.W.Mana:Value() and self.W:IsReady() and not self.Q:IsReady()  then
    ...
    function Syndra:KillSteal(unit)
        for i = 1, #self.enemies do
            ...
            if self.R:IsReady() and self.R:CanCast(unit) and Menu.R.KS:Value() and Menu.R.Heroes[unit.charName] and Menu.R.Heroes[unit.charName]:Value() then
                ...
                if unit.health +75+unit.shieldAP < damage and not Menu.R.KS2:Value()  then
                    self.R:Cast(unit); return
				elseif  ((unit.health+100+unit.shieldAP-damage)/ unit.maxHealth)<0.15  and  Menu.R.KS2:Value()  then
                    self.R:Cast(unit); return
	...
	function Syndra:DisableAAcheck()
    if myHero.levelData.lvl >= Menu.Q.AAlevel:Value()  then
        if Ready(_Q) or Ready(_W) or Ready(_E) or (myHero.levelData.lvl >=Menu.Q.AAlevel2:Value()) then
            orbwalker:SetAttack(false)
        end
    end

    if Menu.Q.AAQ:Value() and Ready(_Q) then
        orbwalker:SetAttack(false)
    end
end
    ...
    function Syndra:CastE(hC, target)
        ...
        if castPos and hitChance >= 1 then
            ...
            if pos and self:CanHitQE(enemy, pos, castPos) then
                ...
                if self.Q:IsReady() and  ( self.mode == 2)  and Menu.QE[enemy.charName] and Menu.QE[enemy.charName]:Value()  then
...
local GetMinions = function(range)
	...
	Q2   = { range = 800 , delay = 0.25, speed = 1500, width = 150, collision = false, aoe = true, type = "linear" } 	
	
			function mundoEcast(target)
				if target == nil then return end
				 
			if  GetDistance(myHero.pos, rtarget.pos)>(520+myHero.boundingRadius) and GetDistance(myHero.pos, rtarget.pos)<900 and (redcast+800)<GetTickCount() then
				...
				if ValidTarget(target, Q2.range) then 
					for i, minion in pairs(GetMinions()) do 
						if minion and not minion.dead and ValidTarget(minion,525+myHero.boundingRadius)  then 
							--local minionPos = LocalVector(myHero.pos):Extended(LocalVector(minion.pos), Q2.range)
							if GetDistance(target.pos, minion.pos) <= 250 then 
								print("looking")
								redcast=GetTickCount()
								cPos = cursorPos
								Control.Attack(minion)
								-- print(cursorPos)
								-- SetCursorPos(cPos)
								-- DelayAction(function() Control.Move(cPos) end, 0.10)
							end
						end
					end
				end
    ...
    function TwistedFate:Combo()
		...
		rtarget=GetTarget(900)
		if HasBuff(myHero, "RedCardPreAttack") then
			mundoEcast(rtarget)
