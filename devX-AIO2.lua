...
class "Swain"
    ...
    function Swain:LoadMenu() --MainMenu
        ...
        self.Menu:MenuElement({type = MENU, id = "Combo", name = "Combo Mode"})
			...
			self.Menu.Combo:MenuElement({id = "QRange", name = "q range", value = 650, min=0, max = 750})
    ...
    function Swain:rootCheck()
        ...
        for i, enemy in pairs(enemies) do
            if isValid(enemy) and not enemy.dead  then
                ...
                if isSpellReady(_W) and self.Menu.Auto.W:Value() and not isSpellReady(_E) and distance <  wSpellData.Range and isImmobile then
    ...
    function Swain:Combo()
        ...
        if target then
            ...
            if isSpellReady(_Q) and self.Menu.Combo.UseQ:Value() then
                if distance < self.Menu.Combo.QRange:Value() then
...
class "RekSai"
    ...
    function Elise:onPostAttack(args)
        ...
        if not self.Menu.Clear:Value() and (_G.SDK.Orbwalker.Modes[_G.SDK.ORBWALKER_MODE_JUNGLECLEAR] or _G.SDK.Orbwalker.Modes[_G.SDK.ORBWALKER_MODE_LANECLEAR]) then
            return
        end
    ...
    function Elise:onTickEvent()
        ...
        if self.Menu.Clear:Value() and (_G.SDK.Orbwalker.Modes[_G.SDK.ORBWALKER_MODE_JUNGLECLEAR] or _G.SDK.Orbwalker.Modes[_G.SDK.ORBWALKER_MODE_LANECLEAR]) then
            self:Clear()
        end
    ...
    function Elise:Combo()
        ...
        local target = _G.SDK.Orbwalker:GetTarget(1200, _G.SDK.DAMAGE_TYPE_MAGICAL)
        if target then
...
function Gnar:__init()	     
    ...
    self:LoadMenu()   
    self.QspellData = {Type = GGPrediction.SPELLTYPE_LINE, Delay = 0.066, Speed = 1200, Range = 1200, Radius = 60, Collision = true, CollisionTypes = {GGPrediction.COLLISION_MINION}}
	self.Q2spellData = {Type = GGPrediction.SPELLTYPE_LINE, Delay = 0.5, Speed = 2100, Range = 1100, Radius = 130, Collision = true, CollisionTypes = {GGPrediction.COLLISION_MINION}}
	self.Q3spellData = {Type = GGPrediction.SPELLTYPE_LINE, Delay = 0.5, Speed = 2100, Range = 1100, Radius = 180, Collision =false }
    self.WspellData = {Type = GGPrediction.SPELLTYPE_LINE, Delay = 0.6, Speed = math.huge, Range = 550, Radius = 100, Collision = false}
    Callback.Add("Draw", function() self:Draw() end) 
    Callback.Add("Tick", function() self:onTickEvent() end)    
end
...
function Gnar:LoadMenu() --MainMenu
	...
	self.Menu.Combo:MenuElement({id = "Q", name = "press to cast q", value = true, toggle = true, key = string.byte("S") })
	self.Menu.Combo:MenuElement({id = "ManualQ", name = "only ever q if pressed", value = false, toggle = true})
end
...
function Gnar:Combo()
    ...
    if target then
        if isSpellReady(_Q) and (self.Menu.Combo.Q:Value() or (myHero.pos:DistanceTo(target.pos) < 450 and not self.Menu.Combo.ManualQ:Value())  ) and not self.isMega  then
            castSpell(self.QspellData, HK_Q, target)
        end

		if isSpellReady(_Q) and  self.isMega and _G.SDK.Orbwalker:CanMove(myHero) and myHero.pos:DistanceTo(target.pos) < 300 then
			castSpell(self.Q3spellData, HK_Q, target)
		end
		if isSpellReady(_Q) and  self.isMega and _G.SDK.Orbwalker:CanMove(myHero) and myHero.pos:DistanceTo(target.pos) > 300 and (self.Menu.Combo.Q:Value() or not self.Menu.Combo.ManualQ:Value()) then
			castSpell(self.Q2spellData, HK_Q, target)
		end

        if isSpellReady(_W) and self.isMega and myHero.pos:DistanceTo(target.pos) < 480 and ((target.charName~= "Fiora") or target:GetSpellData(_W).currentCd>0.3)  then
            castSpell(self.WspellData, HK_W, target)
        end
        if isSpellReady(_E)  then
...
function Zeri:__init()	     
    ...
    _G.SDK.Spell:SpellClear(_Q, 
                            {Range=750, Speed = 2600, Delay = 0.1, Radius = 80 },
                            function ()
...
function Zeri:LoadMenu() --MainMenu
    ...
    self.Menu:MenuElement({ id = "BlockAuto", name = "Block auto without Q passive", value = true ,key=string.byte("T"), toggle = true})
...
function Zeri:onTickEvent()
    ...
    if self.hasPassive and self.Menu.BlockAuto:Value() then
		...
		if target and myHero.pos:DistanceTo(target.pos)<550 then
			_G.SDK.Orbwalker:SetAttack(true)
		end
...
function Zeri:Combo()
    ...
    if target then
        local distance = myHero.pos:DistanceTo(target.pos) 
        if self.hasPassive and distance <= self.aaRange and self.Menu.BlockAuto:Value()  then return end --lets use charged auto instead
        
        if isSpellReady(_W) and self.Menu.WSpell.WEnabled:Value() then
            self:assessWOptions(target)
        end
        if self.Menu.QSpell.QEnabled:Value() and distance < self.qRange and isSpellReady(_Q) and not _G.SDK.Orbwalker:IsAutoAttacking() then
            self:castQ(target)
        end
...
function Zeri:Harass()
    ...
    if target then
        ...
        if self.Menu.QSpell.QHEnabled:Value() and myHero.pos:DistanceTo(target.pos) < self.qRange and isSpellReady(_Q) and not _G.SDK.Orbwalker:IsAutoAttacking()  then
            self:castQ(target)
        end
...
class "Karthus"
    ...
    function Karthus:Combo()
        ...
        if target   then
            ...
            if self.Menu.Combo.E:Value() and isSpellReady(_Q) and distance <= 885 and  self.nextQTimer < Game.Timer() then
				Q = {Type = GGPrediction.SPELLTYPE_CIRCLE, Delay = 0.75, Radius = 160, Range = 875, Speed = math.huge, Collision = false}
                 local QPrediction = GGPrediction:SpellPrediction(Q)
                QPrediction:GetPrediction(target, myHero)
                print(QPrediction:CanHit(1))
        ...
        if not target then
            ...
            if not target then
                ...
                for i, minion in pairs(minions) do
                    ...
                    hp = _G.SDK.HealthPrediction:GetPrediction(minion,self.qSpellData.Delay)
...
class "Rengar"
    ...
    function Rengar:__init()	     
        ...
        _G.SDK.Orbwalker:OnPostAttack(function(...) self:PostAuto(...) end) 
        self.lastMode = ""
    ...
    function Rengar:useQ(target)
        ...
        Control.CastSpell(HK_Q) 
        _G.SDK.Orbwalker:__OnAutoAttackReset()
        DelayAction( function() _G.SDK.Orbwalker:Attack(target) end, 0.12)
    ...
    function Rengar:LaneClear()
        ...
        local jungle = _G.SDK.HealthPrediction:GetJungleTarget()
        if jungle == nil then
            jungle = _G.SDK.HealthPrediction:GetLaneClearTarget()
        end
        if jungle ~= nil then
...
class "Mordekaiser"
    ...
    function Mordekaiser:__init()	     
        ...
        self.qSpell = {Type = GGPrediction.SPELLTYPE_LINE, Delay = 0.4, Radius = 160, Range = 675, Speed = MathHuge, Collision = false}
    ...
    function Mordekaiser:LoadMenu() --MainMenu
        ...
        self.Menu:MenuElement({type = MENU, id = "WSpell", name = "Wlogic/ e+q range sliders"})
            self.Menu.WSpell:MenuElement({id = "Combo", name = "Combo vs Auto", toggle = true, value = true})
            self.Menu.WSpell:MenuElement({id = "WHP", name = "Use W below HP %", value =  10, min=0, max = 100 })
            self.Menu.WSpell:MenuElement({id = "WRange", name = "w range", value = 1200, min=0, max = 2000 })
			self.Menu.WSpell:MenuElement({id = "QRange", name = "q range", value = 650, min=0, max = 670})
			self.Menu.WSpell:MenuElement({id = "ERange", name = "e range", value = 9000, min=0, max = 900})
			self.Menu.WSpell:MenuElement({id = "EExtend", name = "e cast distance behind target", value = 100, min=0, max = 300})
            
        self.Menu:MenuElement({type = MENU, id = "Combo", name = "Combo"})
            self.Menu.Combo:MenuElement({id = "Q", name = "[Q]", value=true}) --toggle = true, value = true,key = string.byte('T')})
            self.Menu.Combo:MenuElement({id = "E", name = "[E]", toggle = false, value = false,key = string.byte('T')})
...
local function Ready(spell)
    ...
    -- Combat Modes
    ---------------------
    function Mordekaiser:ManualE()
	local target = _G.SDK.TargetSelector:GetTarget(1000, _G.SDK.DAMAGE_TYPE_MAGICAL);
        if target then
	
		if self.Menu.Combo.E:Value() and isSpellReady(_E) and myHero.pos:DistanceTo(target.pos) < self.Menu.WSpell.ERange:Value() then
				local pred = GGPrediction:SpellPrediction(self.eSpell)
				pred:GetPrediction(target, myHero)
				if pred:CanHit(GGPrediction.HITCHANCE_NORMAL) then
					castSpellExtended(self.eSpell, HK_E, target, -self.Menu.WSpell.EExtend:Value())
				end
            end			
		end
	end
    ...
    function Mordekaiser:Combo()
        ...
        if target then
            ...
            if self.Menu.WSpell.WHP:Value() and Ready(_W) and myHero.health/myHero.maxHealth * 100 <= self.Menu.WSpell.WHP:Value() then
                Control.CastSpell(HK_W)
            end

            			

            if self.Menu.Combo.Q:Value() and Ready(_Q) and myHero.pos:DistanceTo(target.pos) < self.Menu.WSpell.QRange:Value() then
                local qpred = GGPrediction:SpellPrediction(self.qSpell)
				qpred:GetPrediction(target, myHero)
				Control.CastSpell(HK_Q, qpred.UnitPosition)
            end
