...
Edown = false
    ...
    if Control.IsKeyDown(HK_E) then
        ...
        DelayAction(function() Edown = false end, 0.25)   
    end
end
    ...
    function Viktor:getEPosition(entityTable,target)
		...
		if #entityTable ==1 and target then
			if myHero.pos:DistanceTo(target.pos)<500 then
				local EPrediction = GGPrediction:SpellPrediction({Type = GGPrediction.SPELLTYPE_LINE, Delay = 0.4, Radius = 45, Range = 1100, Speed = math.huge, Collision = false})
				EPrediction:GetPrediction(target, myHero)
				local Pos2 = EPrediction.CastPosition
				local Pos1 = target.pos + (Pos2- target.pos):Normalized() * -150
				Pos1= myHero.pos:Extended(Pos1,math.min(myHero.pos:DistanceTo(Pos1),myHero.pos:DistanceTo(target.pos)))
				return {numHit = 1, startPos = Pos1, endPos = Pos2}
			else 
				local delaytime =(((myHero.pos:DistanceTo(target.pos)-460)/1100))
				local EPrediction = GGPrediction:SpellPrediction({Type = GGPrediction.SPELLTYPE_LINE, Delay = delaytime, Radius = 45, Range = 1100, Speed = math.huge, Collision = false})
				EPrediction:GetPrediction(target, myHero)
			    if EPrediction:CanHit() then
					local endPoint = EPrediction.CastPosition
					
					local directionFromHero = (endPoint - myHero.pos):Normalized()				
					closePoint = myHero.pos + directionFromHero * 480
			...
			for i, entity in pairs(entityTable) do
				for i2, entity2 in pairs(entityTable) do
					-- ((myHero.pos:DistanceTo(entity)-500))
					-- local entityPos1 = entity:GetPrediction(math.huge, 0.25)
					-- local entityPos2 = entity2:GetPrediction(math.huge, 0.25)

					-- local endPoint = nil
					-- local closePoint = nil
					if myHero.pos:DistanceTo(entity.pos) < myHero.pos:DistanceTo(entity2.pos) then 
						local entityPos1 = entity:GetPrediction(math.huge, 0.05)
						local entityPos2 = entity2:GetPrediction(math.huge,(0.05+(entity.pos:DistanceTo(entity2.pos)/1000)))
						endPoint = entityPos2
						closePoint = entityPos1 
					else 
						local entityPos2 = entity2:GetPrediction(math.huge, 0.05)
						local entityPos1 = entity:GetPrediction(math.huge,(0.05+(entity2.pos:DistanceTo(entity.pos)/1000)))
						endPoint = entityPos1 
					...
					for i = 280, 480,100 do -- search range
						...
						if numHit >= bestHit and numHit>0 and hittarget==true then
							bestHit = numHit
    ...
    function Viktor:onTickEvent()
	    ...
	    if E1timer+0.5>Game.Timer() or Control.IsKeyDown(HK_E) then
    ...
    function Viktor:LoadMenu()
        self.Menu = MenuElement({type = MENU, id = "devViktor1", name = "devViktor v1.0"})
