...
local function IsChilled(target)
	for i = 0, target.buffCount do
		...
		if b and b.name == ("aniviachilled" or "aniviaiced") and b.count > 0 and b.duration > 0 then
			return true
		end
	end
	return false
...
local function IsInQ2(unit)
	...
	--if GetDistance(QObject.GameObject.pos, unit.pos) <= QData.Width2 + unit.boundingRadius then
	if  Menu.Combo.Qany:Value() then
		for i, hero in ipairs(GetEnemyHeroes()) do
			if GetDistance(QObject.GameObject.pos, hero.pos) < 220 and (GetDistance(QObject.GameObject.pos, myHero.pos)+50)> GetDistance(myHero.pos, hero.pos) then
				return true
			end
		end
	elseif GetDistance(QObject.GameObject.pos, unit.pos) < 220 and  (GetDistance(QObject.GameObject.pos, myHero.pos)+50)> GetDistance(myHero.pos, unit.pos) then
			return true
...
local function IsSmartChilled(target)
	for i = 0, target.buffCount do
		...
		if b and b.name == ("aniviachilled" or "aniviaiced") and b.count > 0 and b.duration > _time then
			return true
		end
	end
	return false
...
function LoadScript()
	...
	Menu.Combo:MenuElement({id = "UseQ", name = "[Q]key", value = false,key = string.byte("S"), toggle = false })
	Menu.Combo:MenuElement({id = "UseQtoggle", name = "[Q] toggle key", value = false,key = string.byte("Capslock"), toggle = true })
	Menu.Combo:MenuElement({id = "Qany", name = "Q2 any target in range", value = true, toggle = true })
...
--	print(myHero:GetSpellData(_Q).toggleState)
		...
		CheckObjects(Q)
		local target = false or GetHeroTarget(1200)
		if target and IsInQ2(target) then
			Control.CastSpell(HK_Q)
		end
	end
	_G.SDK.Orbwalker:SetAttack(true)
	if Menu.Combo.UseQ:Value() then
	local target = false or GetHeroTarget(1100)
		if target then
			CastQ(target)
		end
	end
...
function CastQ(target)
	...
	if GetDistance(target.pos, myHero.pos) < QData.Range and myHero:GetSpellData(_Q).toggleState == 1 then
		qhc=(Menu.Combo.UseQ:Value() and 1) or 2
		-- if Menu.Pred.Change:Value() == 1 then
		...
		-- else
			...
			if QPrediction:CanHit(qhc) then
