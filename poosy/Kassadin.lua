...
function LoadScript() 	 
	...
	Menu.Combo:MenuElement({id = "UseR", name = "[R]", value = false, toggle=true, key=string.byte("S")})
...
function ComboBasic()
local target = GetHeroTarget(800)
if target == nil then return end
	if IsValid(target) then
        ...
        if Menu.Combo.UseR:Value() and myHero.pos:DistanceTo(target.pos) < 750 and Ready(_R) and myHero.health/myHero.maxHealth >= Menu.Combo.HP:Value() / 100 then
			
			if  _G.SDK.Cursor.Step == 0 then
				_G.SDK.Cursor:Add(string.byte("L"), target.pos)
			end
		end		
...
function ComboBurst()
local target = GetHeroTarget(1200)
if target == nil then return end
...
function LastHit()
    ...
    local minion = GameMinion(i)
        if minion.team == TEAM_ENEMY then
			...
			if Menu.Last.UseQ:Value() and mana_ok and myHero.pos:DistanceTo(minion.pos) < 650 and myHero.pos:DistanceTo(minion.pos) > 400 and IsValid(minion) and Ready(_Q) then
                local QDmg = getdmg("Q", minion, myHero)
				if QDmg >= minion.health then
					Control.CastSpell(HK_Q, minion)
				end	
