...
function LoadScript()
	...
	Menu.Combo:MenuElement({id = "QRange", name = "[Q range extended]", value = 900, min = 800, max = 1100, step = 5})	
	Menu.Combo:MenuElement({id = "UseE", name = "[E]", value = true})		
	Menu.Combo:MenuElement({id = "ERange", name = "[E range extended]", value = 1025, min = 925, max = 1175, step = 5})		
	...
	if IsValid(target) then		
		...
		if Menu.Combo.UseQ:Value() and Ready(_Q) then
			...
			elseif Menu.Pred.Change:Value() == 2 then
				...
				local QPrediction = GGPrediction:SpellPrediction({Type = GGPrediction.SPELLTYPE_CIRCLE, Delay = 0.5, Radius = 235, Range = 800, Speed = 800, Collision = false})
				local Q2Prediction = GGPrediction:SpellPrediction({Type = GGPrediction.SPELLTYPE_CIRCLE, Delay = 0.5, Radius = 100, Range = Menu.Combo.QRange:Value(), Speed = 850, Collision = false})
				QPrediction:GetPrediction(target, myHero)
				Q2Prediction:GetPrediction(target, myHero)
				if Q2Prediction:CanHit(Menu.Pred.PredQ:Value() + 1) then
					if  myHero.pos:DistanceTo(target.pos) <= 785 then
						Control.CastSpell(HK_Q, QPrediction.CastPosition)
					else
						Control.CastSpell(HK_Q,(myHero.pos:Extended(Vector(Q2Prediction.CastPosition), 785)))
					end
				end				
			end
		end
		...
		if Menu.Combo.UseE:Value() and Ready(_E) then			
			...
			elseif Menu.Pred.Change:Value() == 2 then
				...
				local E2Prediction = GGPrediction:SpellPrediction({Type = GGPrediction.SPELLTYPE_CIRCLE, Delay = 0.5, Radius = 250, Range = Menu.Combo.ERange:Value(), Speed = math.huge, Collision = false})
				E2Prediction:GetPrediction(target, myHero)
				if E2Prediction:CanHit(Menu.Pred.PredE:Value() + 1) then
					if  myHero.pos:DistanceTo(target.pos) <= 880 then
						Control.CastSpell(HK_E, E2Prediction.CastPosition)
					else
						Control.CastSpell(HK_E,(myHero.pos:Extended(Vector(E2Prediction.CastPosition), 870)))
					end
				end					
			end	
		end
