local CCSpells = {
	...
	["RenektonExecute"] = {charName = "Renekton", displayName = "Decisive Strike", slot = _W, type = "targeted", range = 225},
	["RenektonSuperExecute"] = {charName = "Renekton", displayName = "Decisive Stri1ke", slot = _W, type = "targeted", range = 225},
	["DariusExecute"] = {charName = "Darius", displayName = "darult", slot = _R, type = "targeted", range = 300, delay = 0.22},
	["DariusAxeGrabCone"] = {charName = "Darius", displayName = "pull", slot = _E, type = "linear", speed = 1600, range = 600, delay = 0.23, radius = 120, collision = true},
	["GarenR"] = {charName = "Garen", displayName = "garen r", slot = _R, type = "targeted", range = 500},
...
function Tick()
	...
	if MyHeroNotReady() then return end
			...
			if Menu.Combo.UseQ2:Value() and Menu.Combo.Active:Value()  then
				if Ready(_Q) then
					PassiveCheck(target)
					for i, Vital in ipairs(PassiveVital) do
						...
						if Vital and Passive then
							...
							if Vital.name == "Fiora_Base_Passive_SE" or Vital.name == "Fiora_Base_Passive_SE_Timeout" then
								local SEPos = Vector(Pos.x - 200 ,Pos.y, Pos.z)	
							...
							if Vital.name == "Fiora_Base_Passive_SW" or Vital.name == "Fiora_Base_Passive_SW_Timeout" then
								local SWPos = Vector(Pos.x ,Pos.y - 200, Pos.z)	
							...
							if Vital.name == "Fiora_Base_Passive_NE" or Vital.name == "Fiora_Base_Passive_NE_Timeout" then
								local NEPos = Vector(Pos.x ,Pos.y + 200, Pos.z)	
							...
							if Vital.name == "Fiora_Base_Passive_NW" or Vital.name == "Fiora_Base_Passive_NW_Timeout" then
								local NWPos = Vector(Pos.x + 200,Pos.y, Pos.z)	
...
function Ult2(unit)
	if unit and HasBuff(unit, "fiorarmark") and (not UltSE or not UltSW or not UltNE or not UltNW) then
...
function ProcessSpell()
	...
	if unit and unit.isEnemy and spell then
		--print(spell.name)
	end
	if unit and unit.isEnemy and spell and CCSpells[spell.name] and Ready(_W) then
		...
		if type == "targeted" then
			if spell.target == myHero.handle then 
				print("trying to block targeted")
