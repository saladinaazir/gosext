...
function LoadScript()
	...
	Menu:MenuElement({id = "General", name = "General Settings", type = MENU})
		...
		Menu.General:MenuElement({id = "CheckInterval", name = "Collision Check Interval", value = 5, min = 5, max = 75, step = 5 })		
...
----------------------------------------------------------------------
	...
	Menu:MenuElement({id = "Skills", name = "Skill Settings", type = MENU})	
		Menu.Skills:MenuElement({id = "Q", name = "Q", type = MENU})
			...
			Menu.Skills.Q:MenuElement({id = "Harass", name = "UseQ Harass/ try to q1 diagonally in combo", value = true })
...
function SemiUlt()
	for i, hero in ipairs(GetEnemyHeroes()) do
		if GetDistance(myHero.pos, hero.pos) <= Menu.Skills.R.Range:Value() and IsValid(hero) and not IsRActive() then
			if Menu.Skills.R.Active:Value() then
				SetMovement(false)
