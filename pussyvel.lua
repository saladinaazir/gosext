...
function SemiUlt()
	for i, hero in ipairs(GetEnemyHeroes()) do
		if GetDistance(myHero.pos, hero.pos) <= Menu.Skills.R.Range:Value() and IsValid(hero) and not IsRActive() and hero== _G.SDK.TargetSelector:GetTarget(1600) then
			if Menu.Skills.R.Active:Value() then
				SetMovement(false)
...
function ControlUlt()
	for i, hero in ipairs(GetEnemyHeroes()) do
		if GetDistance(myHero.pos, hero.pos) <= 1550 and IsValid(hero) and hero== _G.SDK.TargetSelector:GetTarget(1600) then
			if GetDistance(mousePos, hero.pos) > 50 then
		...
		if enemy ~= nil and IsValid(enemy) then			
			...
			if PredPos == false then
				...
				elseif Menu.Pred.Change:Value() == 2 then
					...
					local QPrediction = GGPrediction:SpellPrediction({Type = GGPrediction.SPELLTYPE_LINE, Delay = (0.25+ping), Radius = 35, Range = 1000, Speed = 2100, Collision = false})
					QPrediction:GetPrediction(enemy, myHero)
				...
				if PredPos2 and (GetMode()== "Harass" or GetMode()== "Combo" and Menu.Skills.Q.Combodiagonal:Value()) then
					...
					if Menu.Skills.Q.Harass:Value() and not CheckMinionCollision(myHero, castPos2, Q.Delay, Q.Width, Q.Speed, Q.Range, myHero.pos) and not CheckMinionCollision(myHero, PredPos2, Delay2, Q2.Width, Q2.Speed, Q2.Range, castPos2) then
						Control.CastSpell(HK_Q, castPos2)
						lastSpellCast = Game.Timer()
						PredPos2 = nil	
						Draw.Circle(castPos2, 100, 1, Draw.Color(255, 225, 255, 10))
						return	
					elseif Menu.Skills.Q.Harass:Value() and not CheckMinionCollision(myHero, castPos4, Q.Delay, Q.Width, Q.Speed, Q.Range, myHero.pos) and not CheckMinionCollision(myHero, PredPos2, Delay4, Q2.Width, Q2.Speed, Q2.Range, castPos4) then
...
function DetonateQ()
	if Game.Timer() - qPointsUpdatedAt < .35 and IsQActive() and qHitPoints then
		for i = 1, #qHitPoints do		
			if qHitPoints[i] then		 --CheckMinionCollision(myHero, PredPos2, Delay2, Q2.Width, Q2.Speed, Q2.Range, castPos2) CheckMinionCollision(source, endPos, delay, radius, speed, range, start)
				if qHitPoints[i].playerHit and Menu.Skills.Q.Targets[qHitPoints[i].playerHit.charName] and Menu.Skills.Q.Targets[qHitPoints[i].playerHit.charName]:Value() and not CheckMinionCollision(myHero, qHitPoints[i].playerHit.pos, 0.25, Q2.Width, Q2.Speed, Q2.Range, qMissile.pos) then					
	...
	if Menu.Skills.W.UseW:Value() then
		...
		if enemy ~= nil and IsValid(enemy)  then
			...
			elseif Menu.Pred.Change:Value() == 2 then
				...
				local WPrediction = GGPrediction:SpellPrediction({Type = GGPrediction.SPELLTYPE_LINE, Delay = (0.25), Radius = 87, Range = 1050, Speed = 1700, Collision = false})
	...
	if Menu.Skills.E.UseE:Value() then
		...
		if enemy ~= nil and IsValid(enemy) then
			...
			elseif Menu.Pred.Change:Value() == 2 then
				...
				if EPrediction:CanHit(Menu.Pred.PredE:Value() + 1) and GetDistance(myHero.pos, enemy.pos) <= Menu.Skills.E.Range:Value() and Menu.Skills.E.Targets[enemy.charName] and Menu.Skills.E.Targets[enemy.charName]:Value() then
					lastSpellCast = Game.Timer()
					Control.CastSpell(HK_E, EPrediction.CastPosition)
					return
				end	
				if Menu.Skills.E.TargetSlows:Value() and Menu.Skills.E.Targets[enemy.charName] and Menu.Skills.E.Targets[enemy.charName]:Value() then  AutoERadius(enemy) end
			end
		end
	end
end
...
function AutoERadius(enemy)
	...
	local interceptTime = GetSpellInterceptTime(myHero.pos, targetOrigin, (0.75), E.Speed)			
...
function UpdateQInfo()
	if IsQActive() then	
		local endp=Vector(qMissile.missileData.startPos.x,qMissile.missileData.startPos.y,qMissile.missileData.startPos.z)
		...
		qHitPoints = {}
		local earlydist = Menu.General.earlydist:Value()
		local earlydist2 = 0
		for i = 1, pointCount, 2 do
			local result =  CalculateNode(qMissile,  (qMissile.pos+(endp - qMissile.pos): Normalized() * -earlydist) + directionVector:Perpendicular() * i * checkInterval)			
			qHitPoints[i] = result
			if result.collision then

				break
			end
			local result =  CalculateNode(qMissile,  (qMissile.pos+(endp - qMissile.pos): Normalized() * -earlydist2) + directionVector:Perpendicular() * i * checkInterval)			
			qHitPoints[i+20] = result
			if result.collision then
				--print("failsafeq")
				break
			end
		...
		for i = 2, pointCount, 2 do		
			local result =  CalculateNode(qMissile,  (qMissile.pos+(endp - qMissile.pos): Normalized() * -earlydist) + directionVector:Perpendicular2() * i * checkInterval)		
			qHitPoints[i] = result	
			if result.collision then
				break
			end
						local result =  CalculateNode(qMissile,  (qMissile.pos+(endp - qMissile.pos): Normalized() * -earlydist2) + directionVector:Perpendicular2() * i * checkInterval)			
			qHitPoints[i+20] = result
			if result.collision then
				--print("failsafeq")
				break
