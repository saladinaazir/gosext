...
--MainMenu
self.Menu = MenuElement({type = MENU, id = "zCloverVex", name = "CloverVex"})
...
function Vex:Tick()	
--	print(self.Menu.Misc.Pred.PredQ:Value() + 1)
...
if target == nil then return end
	...
	if IsValid(target) then
		...
		if myHero.pos:DistanceTo(target.pos) <= 1200 and self.Menu.Combo.UseQ:Value() and Ready(_Q)  and not Ready(_E) and (myHero:GetSpellData(_E).cd - myHero:GetSpellData(_E).currentCd)>0.3 then
			if myHero.pos:DistanceTo(target.pos) >= 700  then
				local QPrediction = GGPrediction:SpellPrediction({Type = GGPrediction.SPELLTYPE_LINE, Delay = 0.69, Radius = 80, Range = 1200, Speed = 3200, Collision = false})
					...
					local QPrediction = GGPrediction:SpellPrediction({Type = GGPrediction.SPELLTYPE_LINE, Delay = 0.15, Radius = 160, Range = 1200, Speed = 600, Collision = false})
					QPrediction:GetPrediction(target, myHero)
					if QPrediction:CanHit(self.Menu.Misc.Pred.PredQ:Value() + 1) then
						Control.CastSpell(HK_Q, QPrediction.CastPosition)
					end
		...
		if myHero.pos:DistanceTo(target.pos) <= self.Menu.Combo.Erange:Value() and myHero.pos:DistanceTo(target.pos) >= 800 and  self.Menu.Combo.UseE:Value() and Ready(_E) then
		...
		-- print("looking")
			-- local E1Prediction = GGPrediction:SpellPrediction({Type = GGPrediction.SPELLTYPE_CIRCLE, Delay = 0.25, Radius = 300, Range = 1500, Speed = 1300, Collision = false})
				-- if E1Prediction:CanHit(self.Menu.Misc.Pred.PredE:Value() + 1) then
					-- print("firing")
					-- Control.CastSpell(HK_E, E1Prediction.CastPosition)
				-- end
		-- end
...
if target == nil then return end
	...
	if IsValid(target) then R2ready = myHero:GetSpellData(_R).mana == 0
	...
	if R2ready then Control.CastSpell(HK_R) end
end
